...
from pyportfolio.columns import (
    ...
    TYPE_BUY, TYPE_SELL, TYPE_DIVIDEND, GPP_ALLOWABLE, RESULT_DEFERRED_ADJUSTMENT
)
...
def test_empty_input_dataframe(base_input_columns):
    ...
    expected_calculated_cols = [RESULT_FIFO_GAIN_LOSS, SCRIPT_RCM_COLUMN_NAME, GPP_ALLOWABLE, RESULT_DEFERRED_ADJUSTMENT]
...
def test_simple_gain_and_dividend_scenario(base_input_columns, caplog):
    ...
    assert result_df.loc[0, GPP_ALLOWABLE] == 0.0
    ...
    assert pd.isna(result_df.loc[1, GPP_ALLOWABLE]) # IRPF calc ignores dividends for GPP
    ...
    assert result_df.loc[2, GPP_ALLOWABLE] == pytest.approx(197.0)
...
def test_irpf_loss_deferral_scenario(base_input_columns):
    ...
    assert result_df.loc[1, GPP_ALLOWABLE] == pytest.approx(-104.5)
    assert result_df.loc[1, RESULT_DEFERRED_ADJUSTMENT] == 0.0

    # Row 2 (Blocking BUY)
    assert result_df.loc[2, GPP_ALLOWABLE] == 0.0
...
def test_multiple_groups_and_tickers(base_input_columns):
    ...
    assert result_df.loc[1, GPP_ALLOWABLE] == pytest.approx(9.0)

    # G1, BBB, SELL (index 3)
    assert result_df.loc[3, RESULT_FIFO_GAIN_LOSS] == pytest.approx(-12.0)
    assert result_df.loc[3, GPP_ALLOWABLE] == pytest.approx(-12.0) # Assuming no deferral

    # G2, AAA, SELL (index 5)
    assert result_df.loc[5, RESULT_FIFO_GAIN_LOSS] == pytest.approx(22.4)
    assert result_df.loc[5, GPP_ALLOWABLE] == pytest.approx(22.4)
